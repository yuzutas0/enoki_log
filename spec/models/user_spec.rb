...
describe User do
  ...
  describe "subject associations" do
    ...
    let!(:older_subject) do
      FactoryGirl.create(:subject, user: @user, url: 'abcd12345678' , created_at: 1.day.ago)
    end
    let!(:newer_subject) do
      FactoryGirl.create(:subject, user: @user, url: 'abcdef123456' ,created_at: 1.hour.ago)
    end

    it "should have the right subjects in the right order" do
      expect(@user.subjects.to_a).to eq [newer_subject, older_subject]
    end
